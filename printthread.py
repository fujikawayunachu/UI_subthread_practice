...
def start():
    ...
    # 新しいスレッドを開始
    thread = threading.Thread(target=countdown, name="CountdownThread")
    thread.start()
    print(f"Main Thread: Started CountdownThread")
...
def update_ui():
    ...
    print(f"Main Thread: UI updated")

# 1～10の乱数を発生させる関数
def generate_random_number():
    print(f"{threading.current_thread().name}: generate_random_number() called")
    return random.randint(1, 10)

# 乱数を2倍にする関数
def double_number(number):
    print(f"{threading.current_thread().name}: double_number() called")
    return number * 2
